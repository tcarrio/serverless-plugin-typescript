...
export function getTypescriptConfig(
  ...
  if (fs.existsSync(configFilePath)) {
    ...
    // disallow invalid TypeScript properties for direct invocation
    sanitizeInvalidTsCommandOptions(configParseResult);

    if (logger) {
      logger.log(`Using ${configFilePath}`);
    }

    // disallow overrriding rootDir
    if (
      configParseResult.options.rootDir &&
      path.resolve(configParseResult.options.rootDir) !==
        path.resolve(serviceRoot) &&
      logger
    ) {
      logger.log('Warning: "rootDir" from local tsconfig.json is overriden');
    }
    configParseResult.options.rootDir = serviceRoot;
...
const invalidTsCommandOptions = ["incremental"];
export function sanitizeInvalidTsCommandOptions(config: ts.ParsedCommandLine) {
  invalidTsCommandOptions.forEach(key => {
